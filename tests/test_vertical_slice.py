...
def test_draft_action_activation_and_rollback_vertical_slice() -> None:
    ...
    draft_invocation = system.run_draft_action(action_v1.id, {"name": "n4x"})
    ...
    system.create_test_case(
        rev1.id, action_v1.id, {"name": "test"}, {"message": "hello test"}
    )
    ...
    assert (
        records.source_trees[activated_v1.source_tree_id].status
        == "immutable_snapshot"
    )

    with pytest.raises(ImmutableRevisionError):
        system.run_draft_action(action_v1.id, {"name": "blocked"})

    rev2 = system.create_application_revision(app.id)
    assert system.source.read_source_file(
        rev2.source_tree_id, "actions/hello.py"
    ).content_hash
    assert any(
        action_revision.application_revision_id == rev2.id
        and action_revision.action_id == "tasks.hello"
        and action_revision.source_tree_id == rev2.source_tree_id
        for action_revision in records.action_revisions.values()
    )
    system.source.write_source_file(
        rev2.source_tree_id,
        "actions/hello.py",
    ...
    system.create_test_case(
        rev2.id, action_v2.id, {"name": "test"}, {"message": "hi test"}
    )
...
def test_cloned_schema_definitions_are_replaced_within_one_draft() -> None:
    ...
    original_object = next(
        revision
        for revision in records.object_type_revisions.values()
        if revision.application_revision_id == draft.id
        and revision.object_type_id == "schema.Child"
    )
    original_relation = next(
        revision
        for revision in records.relation_type_revisions.values()
        if revision.application_revision_id == draft.id
        and revision.relation_type_id == "schema.contains"
    )
